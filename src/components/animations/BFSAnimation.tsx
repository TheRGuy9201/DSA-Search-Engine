    ...
    fill={
      isStart 
        ...
        : isCurrentlyVisiting 
          ...
          : isInQueue 
            ? '#8B5CF6' // Purple for queued nodes
            : isVisited 
...
export const BFSAnimation: React.FC = () => {
  // Graph structure - a simple connected graph
  ...
  const bfsPromiseRef = useRef<Promise<void> | null>(null);
  ...
  const startBFS = async () => {
    if (bfsPromiseRef.current) return;
    ...
    bfsPromiseRef.current = (async () => {
      while (bfsQueue.length > 0 && animationRef.current.current) {
        const currentNodeId = bfsQueue.shift()!;
        ...
        if (currentGraphNode) {
          const newNeighbors = currentGraphNode.neighbors.filter(
            neighborId => !visited.has(neighborId) && !bfsQueue.includes(neighborId)
          );
  ...
  return (
    <div className="w-full max-w-5xl mx-auto p-4">
      ...
      <div className="mt-6 text-center text-sm text-gray-400 space-y-3">
        <p className="text-base">Click any node to set it as the starting point for BFS</p>
        ...
        {isCompleted && (
          ...
          >
            BFS traversal complete! Click any node to start from a different position.
          </motion.p>
        )}
      </div>
    </div>
